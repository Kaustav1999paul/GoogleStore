...
public class AddnewAccessoriesActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_addnew_accessories);
    ...
    private void StoreProductInformation(final String imageURL, final String description, final String pname, final String price) {
        ...
        RootRef.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                if (!(dataSnapshot.child("Products").child("Accessories").child(ProductRandomKey).exists())){
                    ...
                    RootRef.child("Accessories").child(ProductRandomKey).updateChildren(productMap).addOnCompleteListener(new OnCompleteListener<Void>() {
                        @Override
                        public void onComplete(@NonNull Task<Void> task) {
                            if (task.isSuccessful()){
                                loadingBar.dismiss();
                                Intent intent = new Intent(AddnewAccessoriesActivity.this, AdminCategoryActivity.class);
                                startActivity(intent);
                                Toast.makeText(AddnewAccessoriesActivity.this, "Item added successfully", Toast.LENGTH_SHORT).show();
                            }else {
                                loadingBar.dismiss();
                                String mess = task.getException().toString();
                                Toast.makeText(AddnewAccessoriesActivity.this, "Error: " + mess, Toast.LENGTH_SHORT).show();
                            }
                        }
                    });
                }
